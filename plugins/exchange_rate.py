...
class Exchanger(threading.Thread):
    ...
    def update_b2c(self):
        jsonresp = self.get_json('www.bit2c.co.il', "/Exchanges/CRYPNIS/Ticker.json")
        return {"NIS": Decimal(str(jsonresp["ll"]))}

    def update_bv(self):
        jsonresp = self.get_json('api.bitcoinvenezuela.com', "/")
        return dict([(r, Decimal(jsonresp["CRYP"][r])) for r in jsonresp["CRYP"]])
    ...
    def update_gc(self):
        ...
        quote_currencies = {}
        for r in jsonresp["prices"]["CRYP"]:
            quote_currencies[r] = Decimal(jsonresp["prices"]["CRYP"][r])
        return quote_currencies

    def update_hb(self):
        quote_currencies = {"EUR": 0.0, "USD": 0.0}
        for cur in quote_currencies:
            quote_currencies[cur] = Decimal(str(self.get_json('api.hitbtc.com', "/api/1/public/CRYP" + cur + "/ticker")["last"]))
        return quote_currencies

    def update_kk(self):
        resp_currencies = self.get_json('api.kraken.com', "/0/public/AssetPairs")["result"]
        pairs = ','.join([k for k in resp_currencies if k.startswith("XCRYPZ")])
    ...
    def request_history_rates(self):
        ...
        if self.cur_exchange == "CoinDesk":
            ...
            if cur_currency in ("ARS", "EUR", "USD", "VEF"):
                try:
                    self.resp_hist = self.exchanger.get_json('api.bitcoinvenezuela.com', "/historical/index.php?coin=CRYP")[cur_currency + '_XVG']
                except Exception:
                    return
            else:
                return
            ...
            if cur_currency in ("EUR", "USD"):
                try:
                    self.resp_hist = self.exchanger.get_json('api.kraken.com', "https://api.kraken.com/0/public/OHLC?pair=CRYP"+cur_currency+"&interval=1440")['result']['XXVGZ'+cur_currency]
